...
y_train=train_data[target_column]                #creating the training target.
...
price_scaler=StandardScaler()  #ONLY scaling the target price, the features are already scaled

y_train_scaled=price_scaler.fit_transform(y_train.values.reshape(-1,1)) #scaling the training prices.
...
with open(os.path.join(output_folder,"regression_model_structure.txt"), "w",
...
history=model.fit(X_train, y_train_scaled, epochs=100, batch_size=32, validation_split=0.2, callbacks=[early_stop], verbose=1)


# 7. Make predictions

y_pred_scaled=model.predict(X_test)  #predicting prices for the test data.

y_pred=price_scaler.inverse_transform(y_pred_scaled) #converting scaled predictions back to real price values.
...
metrics_table=pd.DataFrame({"Metric":["MAE","MSE","RMSE","R2"],
    "Score":[mae,mse,rmse,r2]})

metrics_table.to_csv(os.path.join(output_folder,"regression_metrics.csv"),
    index=False)


# 10. Save predictions

predictions=pd.DataFrame({"actual_price":y_test.values, "predicted_price":y_pred})

predictions["prediction_error"]=predictions["actual_price"]-predictions["predicted_price"]

predictions.to_csv(os.path.join(output_folder,"regression_predictions.csv"), index=False)
...
plt.scatter(predictions["actual_price"], predictions["predicted_price"], color="blue",
    alpha=0.6,
    edgecolors="black",
    s=35)

max_value=max(predictions["actual_price"].max(), predictions["predicted_price"].max())

plt.plot([0,max_value], [0,max_value], color="red", linestyle="--", linewidth=2, label="Ideal line")
...
plt.plot(comparison_data.index, comparison_data["actual_price"], color="blue", linestyle="-",
    linewidth=1,
    alpha=0.7,
    label="Actual Price")

plt.scatter(comparison_data.index, comparison_data["actual_price"], color="blue", marker="o",
    alpha=0.8,
    s=25)

# Predicted price line and markers
plt.plot(comparison_data.index, comparison_data["predicted_price"], color="red", linestyle="--",
    linewidth=1,
    alpha=0.7,
    label="Predicted Price")

plt.scatter(comparison_data.index, comparison_data["predicted_price"], color="red", marker="x", alpha=0.9, s=25)
...
plt.figure(figsize=(8,5))
plt.hist(predictions["prediction_error"], bins=30, color="purple", edgecolor="black")
...
metrics_for_plot=pd.DataFrame({"Metric":["MAE", "RMSE","R2"], "Score":[mae,rmse,r2]})

plt.figure(figsize=(8,5))
plt.bar(metrics_for_plot["Metric"], metrics_for_plot["Score"], color=["blue","orange","green"])
