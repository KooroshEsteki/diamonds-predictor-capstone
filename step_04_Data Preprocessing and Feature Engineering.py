...
df.to_csv(
    os.path.join(output_folder, "feature_engineered_dataset.csv"),
    index=False)
...
df_encoded.to_csv(
    os.path.join(output_folder, "encoded_dataset.csv"),
    index=False)
...
classification_df["clarity_encoded"] = clarity_encoder.fit_transform(
    classification_df["clarity"])

clarity_mapping = pd.DataFrame({
    "clarity": clarity_encoder.classes_,
    "clarity_encoded": range(len(clarity_encoder.classes_))})

clarity_mapping.to_csv(
    os.path.join(output_folder, "clarity_label_mapping.csv"),
    index=False)

X_classification = classification_df.drop(
    columns=["clarity", "clarity_encoded"])
...
classification_train.to_csv(
    os.path.join(output_folder, "classification_train.csv"),
    index=False)

classification_test.to_csv(
    os.path.join(output_folder, "classification_test.csv"),
    index=False)

classification_features = pd.DataFrame({
    "classification_features": X_classification.columns})

classification_features.to_csv(
    os.path.join(output_folder, "classification_features.csv"),
    index=False)
...
regression_train.to_csv(
    os.path.join(output_folder, "regression_train.csv"),
    index=False)

regression_test.to_csv(
    os.path.join(output_folder, "regression_test.csv"),
    index=False)

regression_features = pd.DataFrame({
    "regression_features": X_regression.columns})

regression_features.to_csv(
    os.path.join(output_folder, "regression_features.csv"),
    index=False)
...
clustering_scaled = pd.DataFrame(
    clustering_scaled,
    columns=clustering_df.columns)

clustering_scaled.to_csv(
    os.path.join(output_folder, "clustering_dataset.csv"),
    index=False)

clustering_features = pd.DataFrame({
    "clustering_features": clustering_df.columns})

clustering_features.to_csv(
    os.path.join(output_folder, "clustering_features.csv"),
    index=False)
...
with open(
    os.path.join(output_folder, "preprocessing_report.txt"),
    "w",
    encoding="utf-8"
) as file:
